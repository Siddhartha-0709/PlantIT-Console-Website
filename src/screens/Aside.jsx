...
function Aside(props) {
  ...
  const handleProductUpload = () => {
    navigate('/upload', { state: { userData: userData } })
  }
  const handleBlogUpload = () => {
    navigate('/blogUpload', { state: { userData: userData } })

  }
  const handleContactUs = () => {
    navigate('/contact', { state: { userData: userData } })
  }
  const handleOrders = () => {
    navigate('/orders', { state: { userData: userData } })
  }
  const handleSignOut = () => {
    navigate('/')
  }
  return (
    <aside className="flex h-full min-h-screen w-64 flex-col overflow-y-auto border-r bg-black px-5 py-8" style={{ float: 'left', height: '100%' }}>
      <tr>
        <td>
          <Link to="/" style={{ flexDirection: 'row' }}>
            <svg
              width="40"
              height="46"
              viewBox="0 0 50 56"
        ...
        <td>
          <h1 style={{ color: '#FFFFFF', position:'absolute', top:'45px'}}>{userData.name}</h1>
        </td>
      </tr>
      ...
      <div style={{height:'103px',  borderRadius:'10px',}}>
            <h1 className='font-semibold text-white' style={{marginTop:'80px'}}>PlantIT. Inc</h1>
      </div>
    </aside>
  )
}
